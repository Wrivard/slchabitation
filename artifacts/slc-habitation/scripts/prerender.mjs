...
function createPaidStaticBody(content, routePath) {
  ...
  return `<header class="border-b bg-white"><div class="container-large mx-auto flex items-center justify-between px-6 py-4"><img src="/images/relume-567884.png" width="180" height="60" alt="SLC Habitation"><a href="tel:5144048494">(514) 404-8494</a>${cta}</div></header>
  <main><section class="bg-secondary py-16 text-white"><div class="container-large mx-auto max-w-5xl px-6"><p>${escapeHtml(extra.label)}</p><h1>${escapeHtml(content.h1)}</h1><p>${escapeHtml(content.intro)}</p><ul><li>Licence RBQ : 8351-9033-59</li><li>18 ans d’expérience</li><li>Laval et Laurentides</li></ul>${cta}</div></section>
  <nav aria-label="Navigation dans la page" class="border-b bg-white"><div class="container-large mx-auto px-6 py-4">${nav}</div></nav>
  <section id="${escapeHtml(extra.nav[0][0])}" class="py-16"><div class="container-large mx-auto max-w-4xl px-6"><h2>Une lecture complète de votre projet</h2>${longText}</div></section>
  <section id="${escapeHtml(extra.nav[1][0])}" class="bg-muted py-16"><div class="container-large mx-auto max-w-5xl px-6"><h2>Possibilités et décisions de conception</h2><ul>${configurations}</ul>${images}</div></section>
  <section id="${escapeHtml(extra.nav[2][0])}" class="py-16"><div class="container-large mx-auto max-w-4xl px-6"><h2>Évaluation et planification technique</h2><p>Les points suivants font partie de la lecture initiale; ils sont précisés selon la résidence et le projet envisagé.</p><ul>${evaluation}</ul>${cta}</div></section>
  <section id="${escapeHtml(extra.nav[3][0])}" class="bg-secondary py-16 text-white"><div class="container-large mx-auto max-w-4xl px-6"><h2>Une démarche organisée, du plan aux finitions</h2><p>La coordination relie les choix retenus aux interventions nécessaires et tient compte des conditions observées pendant les travaux.</p><ol><li>Comprendre l’espace et les priorités.</li><li>Mettre le plan en relation avec les éléments techniques.</li><li>Préparer les interventions et les surfaces.</li><li>Coordonner les composantes et les finitions.</li></ol></div></section>
  <section class="py-16"><div class="container-large mx-auto max-w-4xl px-6"><h2>L’expérience d’une cliente</h2><blockquote>« ${escapeHtml(content.testimonial.quote)} »</blockquote><p><strong>${escapeHtml(content.testimonial.author)}</strong>, propriétaire</p></div></section>
  <section id="faq" class="bg-muted py-16"><div class="container-large mx-auto max-w-4xl px-6"><h2>Questions fréquentes</h2><p>Voici des pistes pour préparer votre réflexion. Les réponses précises dépendent de la pièce, du bâtiment et des choix du projet.</p>${faqItems}</div></section>
  <section class="bg-secondary py-16 text-white"><div class="container-large mx-auto max-w-4xl px-6"><h2>Parlons de votre projet</h2><p>Présentez-nous votre espace, vos idées et vos priorités. Nous pourrons amorcer une discussion sur les éléments à examiner.</p>${cta}</div></section></main>
